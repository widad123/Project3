...
contract('SupplyChain', function(accounts) {
    ...
    it("Testing smart contract function buyItem() that allows a distributor to buy coffee", async() => {
       ...
       var price = web3.utils.toWei(String(2), "ether")
        let balance = await web3.eth.getBalance(originFarmerID);
       ...
       // Mark an item as Sold by calling function buyItem()
        ...
        let expectedBalance = await web3.eth.getBalance(originFarmerID);
     ...
     // Verify the result set
      assert.equal(resultBufferOne[1],upc, 'Error: Invalid item UPC')
       ...
       assert.equal(eventEmitted, true, 'Invalid event emitted') 
       assert.equal(resultBufferTwo[4],productPrice,'Error: Invalid productPrice')
       assert.equal(resultBufferTwo[6],distributorID,'Error: Missing or Invalid ditributorID')
        assert.equal(expectedBalance,web3.utils.toBN(balance).add(web3.utils.toBN(productPrice)).toString(),'balance incorrect')
    })    
...
>>>>>>> update
        ...
        assert.equal(resultBufferOne[2], ownerID, 'Error: Missing or Invalid ownerID')
        assert.equal(eventEmitted, true, 'Invalid event emitted') 
        assert.equal(resultBufferTwo[6],distributorID,'Error: Missing or Invalid dtributorID')
    })    
